import{n as T}from"./core-DDXb6Xp5.js";import{g as C,d as H}from"./charts-BQDrhLXk.js";import{s as j,e as p,p as x,a as z}from"./parsers-Bt4hCy--.js";import"./design-system-BaUf1BHL.js";const E=[{key:"cases",key_cn:"案件总览",label:"案件总览"},{key:"timeline",key_cn:"时间线",label:"时间线"},{key:"risks",key_cn:"风险分析",label:"风险分析"}],A=160;let v=null;function F(){v||(v=document.createElement("style"),v.id="legal-detail-styles",v.textContent=`
...
`,document.head.appendChild(v))}function I(o){return`./legal/${o}`}async function N(o){const s={};return await Promise.all(o.map(async r=>{try{const e=await fetch(I(r));e.ok&&(s[r]=await e.text())}catch{}})),s}function L(o){const s=j(o),r={cases:[],timeline:[],risks:[]},e=p(s,"案件列表")||p(s,"案件总览")||"";x(e).rows.forEach(a=>{a.length<3||r.cases.push({caseNo:a[0]||"",name:a[1]||"",type:a[2]||"",status:(a[3]||"").trim(),date:a[4]||"",summary:a[5]||"",amount:a[6]||a[5]||""})});const t=p(s,"时间线")||p(s,"案件时间线")||"";x(t).rows.forEach(a=>{a.length<2||r.timeline.push({date:a[0]||"",event:a[1]||"",type:a[2]||"",caseRef:a[3]||"",note:a[4]||""})});const i=p(s,"风险评估")||p(s,"风险分析")||"";return x(i).rows.forEach(a=>{a.length<2||r.risks.push({factor:a[0]||"",level:a[1]||"",pct:z(a[2])||z(a[1])||0,mitigation:a[3]||a[2]||""})}),r}function P(o){const s=new Date("2026-06-26"),r=`${s.getFullYear()}-${String(s.getMonth()+1).padStart(2,"0")}`;let e=0,n=0,t=0,c=0;return o.forEach(i=>{const d=(i.status||"").trim();/进行中|处理中|审理中|仲裁中/i.test(d)?c++:/已结案|已判决|已裁决|closed/i.test(d)?t++:e++,i.date&&i.date.startsWith(r)&&n++}),{active:e,monthlyNew:n,closed:t,inProgress:c}}function Y(o){const{el:s,cases:r}=o;let e='<div class="legal-fade"><div class="legal-section-title">案件总览</div>';if(r.length===0){e+='<div class="legal-empty"><div class="legal-empty-icon">⚖️</div><div class="legal-empty-text">暂无案件数据</div></div></div>',s.innerHTML=e;return}r.forEach(n=>{const t=(n.status||"").trim();let c,i;/进行中|处理中|审理中|仲裁中/i.test(t)?(c="active",i=t||"进行中"):/已结案|已判决|已裁决|closed/i.test(t)?(c="closed",i=t||"已结案"):(c="pending",i=t||"待处理"),e+=`<div class="legal-case-card">
    ...
    </div>`}),e+="</div></div>";const t=r.filter(i=>(i.pct||0)>=60);let c="";t.length>0?c=`⚠️ 当前有 ${t.length} 个高风险因素需关注：${t.map(i=>i.factor).join("、")}。综合风险评分 ${n}%，建议优先处理。`:c=`✅ 当前整体风险可控，综合风险评分 ${n}%。各风险因素均在可接受范围内。`,e+='<div class="legal-risk-summary">'+c+"</div>",e+="</div>",s.innerHTML=e,setTimeout(()=>{H("legal-gauge",{value:Math.min(n,100),max:100,label:"综合风险"})},50)}function R(){return`
    ...
    </div>`}async function G(o,s,r){var S;F();const e=document.getElementById("header"),n=document.getElementById("site-title");e&&(e.style.background="",e.style.boxShadow=""),n&&(n.innerHTML="⚖️&nbsp;&nbsp;法务看板",n.style.color="");let t=[];try{const l=await fetch("./legal/index.json");l.ok&&(t=await l.json())}catch{}if(t.length===0){o.innerHTML='<div class="legal-empty"><div class="legal-empty-icon">⚖️</div><div class="legal-empty-text">暂无法律案件数据</div></div>';return}o.innerHTML=R();const c=t.filter(l=>l.endsWith(".md")),i=await N(c);let d=[];c.forEach(l=>{const g=i[l];if(!g)return;const u=L(g);d.push(...u.cases)});let a=[],k=[];c.forEach(l=>{const g=i[l];if(!g)return;const u=L(g);a.push(...u.timeline),k.push(...u.risks)});const y=new Set;d=d.filter(l=>{const g=l.caseNo+l.name;return y.has(g)?!1:(y.add(g),!0)});const h=E.find(l=>l.key===r);let m=h?h.key:"cases";const f=P(d);C("legal"),o.innerHTML=`
    <div class="legal-wrap">
    ...
    </div>`,(S=o.querySelector(".legal-back-btn"))==null||S.addEventListener("click",l=>{l.preventDefault(),T("")});const M=o.querySelector("#legal-kpi-strip");M.innerHTML=`
    ...
    <div class="legal-kpi-card"><div class="legal-kpi-label">进行中</div><div class="legal-kpi-value">${f.inProgress}</div></div>`;const b=o.querySelector("#legal-nav");b.innerHTML=E.map(l=>`<div class="legal-nav-item${l.key===m?" active":""}" data-section="${l.key}"><span>${l.label}</span></div>`).join(""),b.querySelectorAll(".legal-nav-item").forEach(l=>{l.addEventListener("click",()=>{b.querySelectorAll(".legal-nav-item").forEach(g=>g.classList.remove("active")),l.classList.add("active"),m=l.dataset.section,T(`#legal/${m}`),$()})});const w=o.querySelector("#legal-content");function $(){const l={el:w,cases:d,timeline:a,risks:k};switch(w.classList.add("legal-fade"),m){case"cases":Y(l);break;case"timeline":_(l);break;case"risks":q(l);break}}$()}export{E as SECTIONS,P as computeKPIs,G as renderLegalDetail};
