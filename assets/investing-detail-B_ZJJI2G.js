import{_ as A,n as M}from"./core-B_Wlt-Ld.js";import{I as T}from"./icons-De6BiXEo.js";import{e as F,f as j,a as q,b as B}from"./charts-BVitNI1m.js";import{e as R,b as y,f as V}from"./parsers-C5lwdAJe.js";import"./design-system-Du44DDix.js";const z=[{key:"overview",icon:T.overview.emoji,label:"市场全景"},{key:"holdings",icon:T.holdings.emoji,label:"持仓分析"},{key:"risk",icon:T.risk.emoji,label:"风险评估"},{key:"strategy",icon:T.strategy.emoji,label:"操作复盘"}],$="investing",D=160;let w=null;function O(){w||(w=document.createElement("style"),w.id="investing-detail-styles",w.textContent=`
/* ─── Layout ─── */
...
`,document.head.appendChild(w))}function N(c){return`./investing/${c}`}async function K(c){const o={};return await Promise.all(c.map(async t=>{try{const i=await fetch(N(t));i.ok&&(o[t]=await i.text())}catch{}})),o}function C(c){let o=c.replace(/\.md$/i,"");return o=o.replace(/^(.+?)_(\d{6})_(.+)$/,"$1($2) $3"),o=o.replace(/_(v\d+(\.\d+)?)$/g,""),o=o.replace(/_\d{4}H\d$/g,""),o=o.replace(/_\d{4}-\d{2}-\d{2}$/g,""),o=o.replace(/_/g," "),o.trim()}function H(c){const o=(c||"").replace(/\s/g,"");return o.includes("上证")||o.includes("沪指")?"上证综指":o.includes("深证")||o.includes("深成")?"深证成指":o.includes("创业")?"创业板指":c||""}function P(c){const o=c||{};let t=0,i=0,r=0,v=0;if(o.holdingsTable&&o.holdingsTable.rows.length>0){const s=o.holdingsTable.rows;let d=0,n=0;s.forEach(e=>{const a=e.find(m=>y(m)!==null),p=a&&y(a)||0;d+=p,p<n&&(n=p),r++}),t=d,v=n}if(o.predictionTable&&o.predictionTable.rows.length>0){let s=0,d=0;o.predictionTable.rows.forEach(n=>{for(const e of n){const a=(e||"").split("/");a.length===2&&(s+=parseInt(a[0])||0,d+=parseInt(a[1])||0)}}),d>0&&(i=Math.round(s/d*100))}return{totalPL:t,winRate:i,tradeCount:r,maxDrawdown:v}}function Y(c,o,t){const i=o||{};let r='<div class="inv-fade">';r+='<div id="inv-date-bar-placeholder"></div>',i.overviewText&&(r+=`<div class="inv-overview-text">${i.overviewText.replace(/\n/g,"<br>")}</div>`),i.indexTable&&i.indexTable.rows.length>0&&(r+='<div class="inv-index-strip">',i.indexTable.rows.forEach(v=>{const s=v[0]||"",d=v[1]||"—",n=(v[2]||"").replace(/[%％]/g,""),e=parseFloat(n)||0,a=e>=0?"pos":"neg",p=e>=0?"▲":"▼";r+=`<div class="inv-index-mini">
    ...
    `,l.querySelectorAll(".inv-date-pill").forEach(b=>{b.addEventListener("click",()=>{e=b.dataset.file,g=m(e);const k=P(g);Object.assign(u,k),f(),I(),E(),S()})}))}function S(){const l=c.querySelector("#inv-nav");l&&(l.innerHTML=z.map(b=>`<div class="inv-nav-item${b.key===p?" active":""}" data-section="${b.key}">
      ...
      </div>`}function E(){const l=c.querySelector("#inv-section-content");if(l){if(!g){l.innerHTML='<div class="inv-empty"><div class="inv-empty-icon">📄</div><div class="inv-empty-text">暂无该报告数据</div></div>';return}switch(p){case"overview":Y(l,g);break;case"holdings":G(l,g);break;case"risk":U(l,g);break;case"strategy":W(l,g);break}}}h()}export{z as SECTIONS,P as computeKPIs,ti as renderInvestingDetail};
